...
def inference(pairs, model, device, batch_size=8, verbose=True, use_amp=False):
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        ...
        # Print the batch to check its structure
        print(f"Batch {i//batch_size + 1}:", batch)
        print(f"Type of batch: {type(batch)}")

        # Check if the batch is a list of dictionaries
        if isinstance(batch, list):
            for element in batch:
                if isinstance(element, dict):
                    # Move each tensor inside the dictionary to the device
                    for k, v in element.items():
                        if isinstance(v, torch.Tensor):  # Ensure we only move tensors
                            element[k] = v.to(device)


        # Use automatic mixed precision (AMP) if enabled
        if use_amp:
            with autocast():  # Automatically casts tensors to float16 where appropriate
