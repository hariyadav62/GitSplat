...
def safe_load(file_path, map_location='cpu'):
    return torch.load(filepath, map_location=map_location)

def load_model(model_path, device, verbose=True, use_half_precision=True):
    if verbose:
    ...
    if verbose:
        print(s)
     # Cast model to half precision (float16) if needed
    '''if use_half_precision:
        net = net.half()  # Convert model parameters to float16'''
    '''for param in net.parameters():
        print(f"Mode Precision: {param.dtype}")'''
    # Convert model parameters to float16 (manually)
    '''if use_half_precision:
        # Convert parameters and buffers to float16
